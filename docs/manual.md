...
stop conditions and halting are bash. That is why there are 27 checks that run free and offline with a stubbed `claude` on `PATH` — including ones for the
...
the loop's own suite in the target** — 27 checks, free and offline, no model.
...
Verified end to end on a Go repo with no Python present: 27 checks pass, and a
