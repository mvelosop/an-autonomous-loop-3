...
- **Every task here adds a scenario, and that moves a number five documents
  ...
  writing the correct values are **38 checks** and **35 scenarios**, T1 having
  already landed the first of the nine; each task after it adds one more of
  each.
...
touch code paths the existing 35 scenarios already exercise — a plan whose
