...
parse, or a non-blank `iterations.jsonl` line that fails to parse — each is a
hard failure. `runstat` never drops the offending record and continues;
undercounting a run's telemetry silently is worse than refusing loudly. The
error names the offending file: the session file's name, or `iterations.jsonl`.
